...
Bootstrap() {
    ...
    if ! eval "$PACSTRAP" -C "$PACMAN_CONF" -c -d -G -M "$SINGULARITY_BUILD_ROOT" haveged $BASE_TO_INST; then
        rm "$PACMAN_CONF"
    ...
    if ! arch-chroot "$SINGULARITY_BUILD_ROOT" /bin/sh -c 'haveged -w 1024; pacman-key --init; pacman-key --populate archlinux'; then
        rm "$PACMAN_CONF"
