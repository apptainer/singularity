...
func init() {
	...
	OciCreateCmd.Flags().StringVar(&ociArgs.LogFormat, "log-format", "kubernetes", "set the format for the log file. The format may be basic, kubernetes or json")
	OciCreateCmd.Flags().SetAnnotation("log-format", "argtag", []string{"<format>"})
	OciCreateCmd.Flags().StringVar(&ociArgs.PidFile, "pid-file", "", "set the path to the pid file")
