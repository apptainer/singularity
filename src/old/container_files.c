...
void update_passwd_file(char *file) {
    ...
    if ( !pwent ) {
        ...
        if ( (errno == 0) || (errno == ESRCH) || (errno == EBADF) || (errno == EPERM) ) {
            singularity_message(VERBOSE3, "Not updating passwd file as entry for %d not found.\n", uid);
            return;
        } else {
    ...
    singularity_message(DEBUG, "Called update_passwd_file(%s)\n", file);

    singularity_message(VERBOSE2, "Checking for passwd file: %s\n", file);
    if ( is_file(file) < 0 ) {
        singularity_message(WARNING, "Template passwd not found: %s\n", file);
        return;
    }

    singularity_message(VERBOSE, "Updating passwd file with user info\n");
...
void update_group_file(char *file) {
    ...
    singularity_message(DEBUG, "Called update_group_file(%s)\n", file);

    singularity_message(VERBOSE2, "Checking for group file: %s\n", file);
    if ( is_file(file) < 0 ) {
        singularity_message(WARNING, "Template group file not found: %s\n", file);
        return;
    }
    if ( ( file_fp = fopen(file, "a") ) == NULL ) { // Flawfinder: ignore
