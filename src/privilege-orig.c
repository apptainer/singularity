...
void update_uid_map(pid_t child, uid_t outside, int is_child) {
    ...
    singularity_message(DEBUG, "Updating UID map.\n");
    if (asprintf(&map_file, "/proc/%i/uid_map", child) < 0) {
        singularity_message(ERROR, "Can't allocate uid map filename\n");
        ABORT(255);
    }
    if (is_child) {
    ...
    if (map_len < 0) {
        free(map_file);
        singularity_message(ERROR, "Can't allocate uid map\n");
        ABORT(255);
    }

    singularity_message(DEBUG, "Updating UID map with policy: %s", map);
    fd = open(map_file, O_RDWR);
    free(map_file);
    if (fd == -1) {
        singularity_message(ERROR, "Failure when opening UID mapfile: %s\n", strerror(errno));
        free(map);
        ABORT(255);
    }
    if (write(fd, map, map_len) != map_len) {
        singularity_message(ERROR, "Failure when writing policy to mapfile: %s", strerror(errno));
        free(map);
        ABORT(255);
    }
    free(map);
...
void update_gid_map(pid_t child, gid_t outside, int is_child) {
    ...
    if (map_len < 0) {
        free(map_file);
        singularity_message(ERROR, "Can't allocate gid map\n");
        ABORT(255);
    }
    if (asprintf(&setgroups_file, "/proc/%i/setgroups", child) < 0) {
        singularity_message(ERROR, "Can't allocate setgroups filename\n");
        ABORT(255);
    }
    singularity_message(DEBUG, "Disabling setgroups file.\n");
    fd = open(setgroups_file, O_RDWR);
    if (fd == -1) {
        free(setgroups_file);
        if (!is_child || (errno != EACCES)) {
            singularity_message(ERROR, "Failure when opening %s: %s\n", setgroups_file, strerror(errno));
            free(map_file);
            free(map);
            ABORT(255);
        }
        ...
        if (write(fd, "deny", 4) != 4) {
            singularity_message(ERROR, "Failure when writing setgroups deny: %s", strerror(errno));
            free(map_file);
            free(map);
            close(fd);
        }
        singularity_message(DEBUG, "Setgroups file successfully disabled.\n");
        close(fd);
    }

    singularity_message(DEBUG, "Updating GID map %s with policy: %s", map_file, map);
    fd = open(map_file, O_RDWR);
    if (fd == -1) {
        singularity_message(ERROR, "Failure when opening GID mapfile (%s): %s\n", map_file, strerror(errno));
        free(map_file);
        free(map);
        exit(-1);
    }
    if (write(fd, map, map_len) != map_len) {
        singularity_message(ERROR, "Failure when writing GID map (%s): %s", map_file, map);
        free(map);
        free(map_file);
        close(fd);
        exit(-1);
...
void priv_init(void) {
    ...
    if ( getuid() == 0 ) {
        ...
        if (target_uid_str) {
            ...
            if (target_uid > 65534) { // Avoid anything greater than the traditional overflow UID.
                singularity_message(ERROR, "Target UID (%ld) cannot be greater than 65534.\n", target_uid);
                ABORT(255);
            }
        }
        if ( !target_uid_str && target_gid_str ) {
        ...
        if (target_gid_str) {
            ...
            if (target_gid > 65534) { // Avoid anything greater than the traditional overflow GID.
                singularity_message(ERROR, "Target GID (%ld) cannot be greater than 65534.\n", target_gid);
                ABORT(255);
            }
        }
    }
    ...
    if ( (target_uid >= 500) && (target_gid >= 500) ) {
        ...
        singularity_message(DEBUG, "Called priv_init(void)\n");
    ...
    singularity_message(DEBUG, "Returning priv_init(void)\n");
}
...
void priv_init_userns_outside() {
#ifdef SINGULARITY_USERNS
    if (!uinfo.ready) {
        singularity_message(ERROR, "Internal error: User NS initialization before general privilege initiation.\n");
        ABORT(255);
    }
    ...
    if (ret == -1) {
        singularity_message(ERROR, "Failed to unshare namespace: %s.\n", strerror(errno));
        ABORT(255);
    }
    ...
    singularity_message(ERROR, "Internal error: User NS function invoked without compiled-in support.\n");
    ABORT(255);
#endif  // SINGULARITY_USERNS
}
...
void priv_init_userns_inside_init() {
    ...
    if (!uinfo.userns_ready) {
        singularity_message(ERROR, "Internal error: User NS privilege data structure not initialized.\n");
        ABORT(255);
    }
...
void singularity_priv_drop_perm(void) {
    ...
    if ( geteuid() == 0 ) {
        if ( !uinfo.userns_ready ) {
            ...
            singularity_message(DEBUG, "Not resetting supplementary groups as we are running in a user namespace.\n");
        }
        ...
        singularity_message(DEBUG, "Running as root, no privileges to drop\n");
    }

    singularity_message(DEBUG, "Confirming we have correct GID\n");
    if ( getgid() != uinfo.gid ) {
        singularity_message(ERROR, "Failed to drop effective group privileges to uid %d: %s\n", uinfo.uid, strerror(errno));
        ABORT(255);
    }
    ...
    singularity_message(DEBUG, "Returning singularity_priv_drop_perm(void)\n");
}
