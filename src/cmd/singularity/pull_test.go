...
func TestPullForce(t *testing.T) {
	...
	if b, err := imagePull(pullOpts{}, "library://davet-sylabs/linux/ubuntu:17.10"); err != nil {
		t.Log(string(b))
	...
	if b, err := imagePull(pullOpts{force: true}, "library://davet-sylabs/linux/ubuntu:17.10"); err != nil {
		t.Log(string(b))
		t.Fatalf("unexpected failure: %v", err)
	}
	imageVerify(t, imagePath, false)
