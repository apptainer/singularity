...
// Parse a --mount string into a BindPath struct.
//
...
// binding out of a SIF partition).
//
// We use a CSV reader to parsing the fields in a mount string according to CSV
// escaping rules. This is the approach docker uses to allow special characters
// in source / dest etc., and we wish to be as compatible as possible.
//
...
func ParseMountString(mount string) (bp BindPath, err error) {
	...
	if len(records) != 1 {
		return BindPath{}, fmt.Errorf("each mount string must specify exactly one mount")
	}
