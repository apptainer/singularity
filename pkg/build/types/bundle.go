...
type Bundle struct {
	...
	TempDir    string `json:"tmpPath"`    // where temp files required during build will appear
}
...
type Options struct {
	...
	// TempDir specifies a non-standard temporary location to perform a build.
	TmpDir string
...
func (b *Bundle) Remove() error {
	var errors []string
	for _, dir := range []string{b.TempDir, b.RootfsPath} {
